 ...
 * 2.5添加血块功能，吃了满血
 * 2.6敌方坦克全灭时再次加入，主战坦克死亡后f2复活
 * 2.7修正了direction等不合理的地方
 * 2.8添加了爆炸图片模型、坦克模型（待修改）、子弹模型
 * 2.9添加配置文件，方便改变配置量，应用单例模型
*/
import java.awt.*;
...
public class TankClient extends Frame {
	...
	BloodBag bag = new BloodBag(this);
	...
	public void launchFrame() {
		for (int i = 0; i < Integer.parseInt(PropertyMgr.getProperty("initTankCount")); i++) {
			enemyTank.add(new Tank(50 + 40 * (i + 1), 50, false, Direction.D, this));
		}
		for (int i = 0; i < 20; i++) {
	...
	public void paint(Graphics g) {
		...
		if(enemyTank.size() == 0){
			for (int i = 0; i < Integer.parseInt(PropertyMgr.getProperty("reTankCount")); i++) {
				enemyTank.add(new Tank(50 + 40 * (i + 1), 50, false, Direction.D, this));
			}
		}
