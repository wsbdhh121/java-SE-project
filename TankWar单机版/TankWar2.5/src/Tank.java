...
public class Tank {
	...
	private Missile fire(Direction dir){
		...
		Missile m;
		if(this.bGood){
			m = new Missile(x1, y1, dir, true, this.tc);
		}else{
			m = new Missile(x1, y1, dir, false, this.tc);
		}
		tc.missiles.add(m);
		return m;
	}
	...
	public boolean eatBag(BloodBag bag){
		if(this.live && bag.isLive() && this.getRect().intersects(bag.getRect())){
			this.life = 100;
			bag.setLive(false);
			return true;
		}
